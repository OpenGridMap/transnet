    ...
    def retrieve_relations(self):
        ...
        for (parts,) in result:
            ...
            for part in parts:
                ...
                if 'station' in type:
                    sql = "select id,create_polygon(id) as geom, hstore(tags)->'power' as type, hstore(tags)->'name' as name, hstore(tags)->'ref' as ref, hstore(tags)->'voltage' as voltage, nodes,tags from planet_osm_ways where id = " + str(part)
                    self.cur.execute(sql)
                    [(id, geom, type, name, ref, voltage, nodes, tags)] = self.cur.fetchall()
                    polygon = wkb.loads(geom, hex=True)
                    relation.append(Station(id, polygon, type, name, ref, voltage, nodes, tags))
                elif 'generator' in type or 'plant' in type:
                    sql = "select id,create_polygon(id) as geom, hstore(tags)->'power' as type, hstore(tags)->'name' as name, hstore(tags)->'ref' as ref, hstore(tags)->'voltage' as voltage, hstore(tags)->'generator:output' as output, nodes,tags from planet_osm_ways where id = " + str(part)
                    self.cur.execute(sql)
                    [(id, geom, type, name, ref, voltage, output, nodes, tags)] = self.cur.fetchall()
                    polygon = wkb.loads(geom, hex=True)
                    generator = Station(id, polygon, type, name, ref, voltage, nodes, tags)
                    generator.nominal_power = output
                    relation.append(generator)
                elif 'line' in type or 'cable' in type:
                    sql =   "select id, create_line(id) as geom, hstore(tags)->'power' as type, hstore(tags)->'name' as name, hstore(tags)->'ref' as ref, hstore(tags)->'voltage' as voltage, hstore(tags)->'cables' as cables, nodes, tags from planet_osm_ways where id = " + str(part)
                    self.cur.execute(sql)
                    [(id, geom, type, name, ref, voltage, cables, nodes, tags)] = self.cur.fetchall()
                    line = wkb.loads(geom, hex=True)
                    relation.append(Line(id, line, type, name, ref, voltage, cables, nodes, tags))
                else:
